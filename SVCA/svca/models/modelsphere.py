...
class ModelSphere(Model):
    """docstring for ModelSphere."""
    def __init__(self, Y, positions, radii, types=None, norm='quantile',
                 oos_predictions=0., image_ids=None, cov_terms=None, kin_from=None, cv_ix=None):
        super(ModelSphere, self).__init__()
        ...
        if cov_terms is None:
            cov_terms = ['intrinsic', 'interactions', 'environmental', 'global']
    ...
    def build_image(self):
        assert self.image_ids is not None, 'Image Ids needed to compute image kinship'
        ImageKin = utils.build_cell_type_kinship(self.image_ids)
        K = ImageKin[self.train_set,:][:,self.train_set]
        K_cross = ImageKin[~self.train_set, :][:, self.train_set]
    ...
    def build_cov(self, cov_terms):
        ...
        self.global_cov = self.build_global()
        ...
        if 'global' in cov_terms:
            self.covar_terms['global'] = self.global_cov
        if 'images' in cov_terms:
            self.image_cov = self.build_image()
            self.covar_terms['images'] = self.image_cov
    ...
    def build_global(self):
        Xtrain, Xstar = self.R[self.train_set, :], self.R[~self.train_set, :]
        if Xstar.shape == (0,0): Xstar = None
        environmental_cov = SQExpCov(Xtrain, Xstar=Xstar)
        environmental_cov.act_length = False
        return environmental_cov
    ...
    def train_gp(self, grid_size=20, train_global=False):
        ...
        if train_global:
            self.global_cov.length = best_l**2.
        try:
